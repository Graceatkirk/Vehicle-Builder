...
async function createVehicle() {
    ...
    const commonQuestions = [
        ...
        },
        {
            ...
            validate: (input) => !isNaN(Number(input)) || 'Please enter a valid year.',
        },
        {
            type: 'input',
            name: 'weight',
            message: 'What is the weight of the vehicle?',
            validate: (input) => !isNaN(Number(input)) || 'Please enter a valid weight.',
        },
        {
            type: 'input',
            name: 'maxSpeed',
            message: 'What is the maximum speed of the vehicle?',
            validate: (input) => !isNaN(Number(input)) || 'Please enter a valid maximum speed.',
        },
    ];
    ...
    switch (vehicleType) {
        case 'Car':
            newVehicle = new Car(Cli.generateVin(), commonAnswers.color, commonAnswers.make, commonAnswers.model, Number(commonAnswers.year), Number(commonAnswers.weight), Number(commonAnswers.maxSpeed), []);
            break;
        case 'Truck':
            newVehicle = new Truck(Cli.generateVin(), commonAnswers.color, commonAnswers.make, commonAnswers.model, Number(commonAnswers.year), Number(commonAnswers.weight), Number(commonAnswers.maxSpeed), [], 10000 // Example towing capacity
            );
            break;
        case 'Motorbike':
            ...
            newVehicle = new Motorbike(Cli.generateVin(), commonAnswers.color, commonAnswers.make, commonAnswers.model, Number(commonAnswers.year), Number(commonAnswers.weight), Number(commonAnswers.maxSpeed), motorbikeWheels);
            break;
        default:
...
async function performAction(vehicle) {
    const { action } = await inquirer.prompt([
        {
            type: 'list',
            name: 'action',
            message: `What would you like to do with your ${vehicle.make} ${vehicle.model}?`,
            choices: ['Start', 'Display Info', 'Back'],
        },
    ]);
    switch (action) {
        case 'Start':
            console.log(`${vehicle.make} ${vehicle.model} is starting.`);
            break;
        case 'Display Info':
